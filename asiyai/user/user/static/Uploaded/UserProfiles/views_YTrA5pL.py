...
def hvregistration(request):
    if request.method=="POST":
            ...
            # hello = heavyvehivalregistration.objects.create(vehicle_image=vehical_name)
            # hello.save()
            db = heavyvehivalregistration(vehical_name=vehical_name,company_name=company_name,vehical_number=vehical_number,manufacture_date=manufacture_date,ownername=ownername,Aadhar_number=Aadhar_number,alternativemobilenumber=alternativemobilenumber, vehicle_image=vehicle_image)
            db.save()
            #return HttpResponse(request,"Basic info successfully")
...
def showhvone(request, Id):
    ...
    if request.method=="POST":
        ...
        db = Request_Heavy_Vehical(vehical_name=hv.vehical_name,company_name=hv.company_name, vehical_number=hv.vehical_number,ownername=hv.ownername, Aadhar_number=hv.Aadhar_number,vehicle_image=hv.vehicle_image,manufacture_date=hv.manufacture_date,alternativemobilenumber=hv.alternativemobilenumber, created_by = request.user.id)
        db.save()
        data = {
         "hv": hv,
...
def showdrone(request, Id):
    ...
    if request.method=="POST":
        dr = driveroperatorregistration.objects.filter(Id=Id).first()
        db = Request_driver_Operator(vehicalname=dr.vehicalname, expriencesinyear=dr.expriencesinyear, driveroperatorname=dr.driveroperatorname,Aadhar_number=dr.Aadhar_number, alternet_mobilenumber=dr.alternet_mobilenumber, license_number=dr.license_number, driver_image=dr.driver_image, created_by = request.user.id)
        db.save()
        data1 = {
            "dr": dr,
...
def subsingle(request, Id):
    ...
    if request.method=="POST":
        subcon = subcontractorregistration.objects.filter(Id=Id).first()
        db = Request_SubContractor(contractorname=subcon.contractorname, firmname=subcon.firmname, expriencesinyear=subcon.expriencesinyear, license_number=subcon.license_number, Aadhar_number=subcon.Aadhar_number, subcontractor_image=subcon.subcontractor_image, created_by = request.user.id)
        db.save()
        sub = {
            "subcon": subcon,
...
def laboursingle(request, Id):
    ...
    if request.method=="POST":
        ...
        db = Request_labour_contructor(labourcontractorname=lab.labourcontractorname, labourwork=lab.labourwork, lobourinnumber=lab.lobourinnumber, contractorAadhar_number=lab.contractorAadhar_number, mobile_number=lab.mobile_number, labour_image=lab.labour_image, created_by = request.user.id)
        db.save()
        var = {
            "lab": lab,
...
def doregistration(request):
    if request.method=="POST":
        ...
        #hello = driveroperatorregistration.objects.create(driver_image=driver_image)
        db = driveroperatorregistration(vehicalname=vehicalname, expriencesinyear=expriencesinyear, driveroperatorname=driveroperatorname,Aadhar_number=Aadhar_number, alternet_mobilenumber=alternet_mobilenumber, license_number=license_number, driver_image=driver_image)
        db.save()
        return redirect("/v1/hvaddress")
    else:
...
def subcregistration(request):
    if request.method=="POST":
        ...
        # Hello = subcontractorregistration.objects.create(subcontractor_image=subcontractor_image)
        # Hello.save()
        db = subcontractorregistration(contractorname=contractorname, firmname=firmname, expriencesinyear=expriencesinyear, license_number=license_number, Aadhar_number=Aadhar_number, subcontractor_image=subcontractor_image,)
        db.save()
        return redirect("/v1/hvaddress")
    else:
...
def lacoregistration(request):
    if request.method=="POST":
        ...
        v_contractorAadhar_number = request.POST.get('contractorAadhar_number')
        v_mobile_number = request.POST.get('mobile_number')
        labour_image = request.FILES['labour_image']
        db = labour_contructor(labourcontractorname=v_labourcontractorname, labourwork=v_labourwork, lobourinnumber=v_lobourinnumber, contractorAadhar_number=v_contractorAadhar_number, mobile_number=v_mobile_number, labour_image=labour_image,)
        db.save()
        return redirect("/v1/hvaddress")
...
def hvadddress(request):
    if request.method=="POST":
        ...
        db = MasterContents.objects.get(country=country, state=state, district=district)
        if db:
